...
test.describe('watch form validation', () => {
  test('should watch all inputs', async ({ page }) => {
    ...
    await expect(page.locator('#HideTestSingle')).toContainText('Hide Content TestSingle');
    await expect(page.locator('#watchAll')).toContainText(JSON.stringify({
    ...
    await expect(page.locator('#testArray')).toContainText(JSON.stringify(['bill', 'luo']));
    ...
    await expect(page.locator('#testObject')).toContainText(JSON.stringify({
      firstName: 'bill',
      lastName: 'luo',
    }));
