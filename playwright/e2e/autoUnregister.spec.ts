...
test.describe('autoUnregister', () => {
  test('should keep all inputs data when inputs get unmounted', async ({ page }) => {
    ...
    await expect(page.locator('#input-ReactSelect > div > div > div > div').first()).toContainText('Strawberry');
  });
});
